...
export class ProfileController {
    ...
    async getUserUuidByUsername(req: Request, res: Response) {
        try {
            ...
            const userUuid = await this.service.getUserUuidByUsername(
                username
            );
            if (!userUuid) {
                return res.status(404).json({
        ...
        try {
            ...
            const email =
                body && typeof body.email === "string"
                    ...
                    : undefined;
            const fullName =
                body && typeof body.fullName === "string"
                    ? body.fullName
                    : undefined;
    ...
    async update(req: Request, res: Response) {
        try {
            ...
            if (
                ...
                !["ACTIVE", "INACTIVE", "BANNED", "BUSY"].includes(
                    filteredData.status
                )
            ) {
                validationErrors.push(
            ...
            if (filteredData.telegramId !== undefined) {
                const telegramId = parseInt(
                    String(filteredData.telegramId),
                    10
                );
                if (isNaN(telegramId) || telegramId <= 0) {
                    validationErrors.push(
                        "telegramId must be a positive number"
                    );
                } else {
